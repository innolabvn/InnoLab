...
def main() -> None:
    ...
    try:
        ...
        sum_input_token, sum_output_token, sum_total_token = 0, 0, 0
        ...
        for i, iteration in enumerate(result.get("iterations", []), 1):
            ...
            total_token = fix_result.get("total_tokens", 0)
            if total_token > 0:
                input_token = fix_result.get('total_input_tokens', 0)
                output_token = fix_result.get('total_output_tokens', 0)
                logger.info("\nToken Usage:")
                logger.info(f"Input tokens: {input_token:,}")
                logger.info(f"Output tokens: {output_token:,}")
                logger.info(f"Total tokens: {total_token:,}")
                ...
                sum_input_token += input_token
                sum_output_token += output_token
                sum_total_token += total_token

            if fix_result.get("message"):
                logger.info(f"Message: {fix_result.get('message')}")

        
        logger.info(f"Total token usage: {sum_total_token:,}, Input: {sum_input_token:,}, Output: {sum_output_token:,}")
        logger.info(f"Start time: {result.get('start_time')}")
        logger.info(f"End time: {result.get('end_time')}")

        # JSON cuối để machines parse
        logger.info("END_EXECUTION_RESULT")
