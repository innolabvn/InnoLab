...
class BearerScanner(Scanner):
    ...
    def __init__(self, config: dict = None):
        ...
        self.bearer_path = self.config.get("bearer_path", "bearer")
        self.timeout = self.config.get("timeout", 300)
        self.project_key = None
        self.scan_directory = None
    
    def scan_project(self, project_path: str, **kwargs) -> dict:
        """Run Bearer scan on a specific project path."""
        self.project_key = os.path.basename(project_path)
        self.scan_directory = project_path
        
        # Run the scan and convert to expected format
